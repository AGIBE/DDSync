...
class Geoproduct(object):
    ...
    def extract_dd_infos(self):
        ...
        if not self.gpr_exists:        
            self.sql_statements.append("INSERT INTO %s.TB_GEOPRODUKT (gpr_bezeichnung, gpr_bezeichnung_mittel_de, gpr_bezeichnung_mittel_fr, gpr_bezeichnung_lang_de, gpr_bezeichnung_lang_fr, the_objectid, gpr_objectid) VALUES ('%s', '%s', '%s', '%s', '%s', %s, %s)" % (dd_schema, self.gpr_bezeichnung, clean(self.gpr_bezeichnung_mittel_de), clean(self.gpr_bezeichnung_mittel_fr), clean(self.gpr_bezeichnung_lang_de), clean(self.gpr_bezeichnung_lang_fr), self.the_objectid, self.gpr_objectid))
